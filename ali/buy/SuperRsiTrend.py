...
class MyStrategy:
    ...
    def calculate_signals_1(self):
        ...
        if rsi_3m.iloc[-3] < 30 and df_15m['close'].iloc[-3] > supertrend_15m.iloc[-3] * 0.99:
            # 这里用的小时图的值往下2%
            strategy_name = "2_1"
            self.update_position(strategy_name, 1)  # 买入信号
            print(f"更新了信号 {strategy_name}: {self.signals[strategy_name]}")  # 打印以确认
        #   卖出逻辑： 30分super跌破
        if rsi_15m.iloc[-3] > 75 or df_15m['close'].iloc[-2] < supertrend_15m.iloc[-3] < df_15m['close'].iloc[-3]:
