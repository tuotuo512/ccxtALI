...
class MyStrategy:
    ...
    def set_data(self, df_1m, df_5m, df_15m, df_30m, df_60m):
        self.dataframes = {
            '1m': df_1m,
            '5m': df_5m,
            '15m': df_15m,
    ...
    def calculate_signals_1(self):
        ...
        supertrend_60m = self.indicators['supertrend_60m']
        ...
        print('60分钟轨道值：', supertrend_60m.iloc[-2])  # 打印super上轨参照

        rsi_15m = self.indicators['rsi_15m']
        rsi_60m = self.indicators['rsi_60m']
        ...
        #   1_3. 顺势加仓逻辑: 15m进15m出  但是价格大于小时图
        if supertrend_60m.iloc[-2] < df_15m['close'].iloc[-3] < supertrend_15m.iloc[-3] < df_15m['close'].iloc[-2]:
            strategy_name = "1_3"  # 根据update_position中的参数来构造策略名称
            self.update_position(strategy_name, 1)  # 买入信号
            print(f"更新了信号 {strategy_name}: {self.signals[strategy_name]}")  # 打印以确认
            # 1_4平仓逻辑：15分 小时图图super下穿
        if df_15m['close'].iloc[-2] < supertrend_15m.iloc[-3] < df_15m['close'].iloc[-3] or supertrend_60m.iloc[-2] > \
                df_15m['close'].iloc[-3]:
            strategy_name = "1_3"  # 根据update_position中的参数来构造策略名称
            self.update_position(strategy_name, -1)  # 买入信号
            print(f"更新了信号 {strategy_name}: {self.signals[strategy_name]}")  # 打印以确认

        #   1_4. 反向买入逻辑：15分破位进，15分出，小时图收盘价大于sup
        if (df_15m['close'].iloc[-3] > supertrend_15m.iloc[-3] > df_15m['close'].iloc[-2]) \
                and (df_15m['close'].iloc[-2] > supertrend_60m.iloc[-2]):
            strategy_name = "1_4"  # 根据update_position中的参数来构造策略名称
            self.update_position(strategy_name, 1)  # 买入信号
            print(f"更新了信号 {strategy_name}: {self.signals[strategy_name]}")  # 打印以确认
        #   卖出逻辑：按照半小时图
        if rsi_15m.iloc[-3] > 80 or supertrend_60m.iloc[-2] > df_15m['close'].iloc[-3]:  # 小于suer止损
            strategy_name = "1_4"  # 根据update_position中的参数来构造策略名称
            self.update_position(strategy_name, -1)  # 买入信号
            print(f"更新了信号 {strategy_name}: {self.signals[strategy_name]}")  # 打印以确认

        #   1_5. 反向买入逻辑：15分下穿买入，价格大于小时图sup，  小时超买后，微小级别卖出
        if (df_15m['close'].iloc[-3] > supertrend_15m.iloc[-3] > df_15m['close'].iloc[-2]) \
                and (df_15m['close'].iloc[-2] > supertrend_60m.iloc[-2]):
            ...
            print(f"更新了信号 {strategy_name}: {self.signals[strategy_name]}")  # 打印以确认
        #   卖出逻辑：小时超买后，微小级别卖出
        if ((rsi_60m.iloc[-3] > 75 and df_5m['close'].iloc[-2] < supertrend_5m.iloc[-3] < df_5m['close'].iloc[-3])
                or supertrend_60m.iloc[-2] > df_15m['close'].iloc[-3]):  # 小于suer止损
            strategy_name = "1_5"  # 根据update_position中的参数来构造策略名称
            self.update_position(strategy_name, -1)  # 买入信号
            print(f"更新了信号 {strategy_name}: {self.signals[strategy_name]}")  # 打印以确认
