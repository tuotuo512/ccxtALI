...
def manual_update_positions():
    ...
    initialize_positions['1_1'] = (0, 0)  # 15这里是手动填入 目前仓位持仓 1-1
    initialize_positions['1_2'] = (0, 0)  # 30分进出                  1-2
    initialize_positions['1_3'] = (0, 0)  # 60进 m出            1-3
    initialize_positions['1_4'] = (0, 0)  # 进 30m出              1-4
    initialize_positions['1_5'] = (0, 0)  # 进 30m出              1-4
...
def run():
    ...
    while True:
        ...
        # 每次循环时获取最新数据
        df_1m, df_5m, df_15m, df_30m, df_60m = fetch_and_process_market_data(exchange, historical_df)

        # print(df_15m.tail(5))

        strategy = MyStrategy()
        strategy.set_data(df_1m, df_5m, df_15m, df_30m, df_60m)  # 设置策略数据
